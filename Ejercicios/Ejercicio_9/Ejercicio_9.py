...
if __name__ == "__main__":
    ...
    first_face_in_range = ms.get_isosurface_triangle(mesh, MIN_VALUE, MAX_VALUE)

    # Recorremos la malla desde la cara anterior, obteniendo todas las caras que se encuentran en rango
    faces_in_range = ms.get_in_range_faces(mesh, first_face_in_range, MIN_VALUE, MAX_VALUE, [])

    # Con las caras en rango, creamos una nueva malla
    new_mesh = ms.create_new_mesh(faces_in_range, mesh)

    # Obtenemos los vertices e indices de la nueva malla
    isosurface_vertices, isosurface_indices = ms.get_vertexs_and_indexes(new_mesh)
    ...
    while not glfw.window_should_close(window):
        ...
        glUniformMatrix4fv(glGetUniformLocation(pipeline.shaderProgram, "model"), 1, GL_TRUE, tr.uniformScale(2))
        pipeline.drawCall(gpuCubo)
